...
| `quarantine_state` | 否 | none、quarantined、released、purged |
| `retry_count` | 否 | 清理重试次数 |
| `last_failure_code` | 是 | 最近一次失败原因 |
| `retention_until` | 是 | 保留到期时间；无保留策略时为空 |
...
6. 最后引用删除后，内容先进入 `pending_cleanup` 或隔离区；保留期内重新建立引用时必须校验账本、权限、哈希和版本。
7. 隔离区内容不可被普通下载接口访问，只能由清理任务、恢复引用预览或管理员受控流程读取元数据。
8. 清理任务必须幂等；重复清理同一内容返回原结果，不重复删除墓碑或审计。
9. 清理失败可按退避策略重试；超过重试上限后进入 `cleanup_failed` 并保留可解释状态。
10. 云端物理清理不得远程删除 PC 本地未同步附件文件；PC 本地受管附件清理由 PC 本机用户或本地任务确认执行。
...
5. 从隔离区恢复引用时必须重新校验 `expected_content_hash`、`reference_version` 和账本权限；哈希不一致必须阻断。

附件文件生命周期错误码至少包括：

| 错误码 | 说明 |
| --- | --- |
| `attachment_cleanup_retry_exhausted` | 物理清理多次失败，进入人工或后台处理状态 |
| `attachment_quarantine_not_downloadable` | 隔离区内容不允许普通下载 |
| `attachment_restore_hash_mismatch` | 恢复引用时内容哈希不一致 |
| `attachment_local_cleanup_required` | PC 本地受管文件需要本机确认清理 |
| `attachment_cleanup_remote_local_forbidden` | 云端任务试图删除 PC 本地文件 |
...
15. 隔离区内容不能通过普通下载接口访问；恢复引用必须重新校验哈希、权限和引用版本。
16. 云端清理任务不能远程删除 PC 本地未同步附件文件；PC 本地清理必须由本机用户或本地任务确认。
