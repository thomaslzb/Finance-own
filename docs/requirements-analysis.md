...
同日 `RT-15-009` 普通提醒补充样例又确认：一次性 `CodexRT15CalendarReminder` 保存后，同时进入计划列表、今日提醒和财务日历；日历日期格使用独立勾选图标，右侧只显示提醒名称，没有观察到“查看”入口，并与“有账务记录”独立共存。后续未来每天重复 2 次的 `CodexRT15FutureDaily` 区分了两个查询口径：业务日 `2026-08-03` 的今日提醒已按提前窗口显示第一期 `2026-08-04`，但日历只在定义开始日 `2026-08-04` 显示；跳过第一期后，今日提醒和计划下次日期推进到 `2026-08-05`，日历却仍只在 `2026-08-04` 显示，且冷启动后保持；最终跳过使定义完成后，开始日标记才消失。因此 Rust 版必须分离 `ReminderSchedule`、`ReminderOccurrence` 和 `LegacyReminderCalendarProjection`：今日提醒以稳定 `occurrence_id` 按资格窗口推进，旧版兼容日历以 `schedule_id` 把活动定义锚定在 `start_date`。跳过原子保存真实 `skipped` 动作、处理时间和审计；`start_date`、`occurrence_date` 与 `eligible_from_date` 必须分别保存。详细合同见 `runtime-reminder-calendar-contract.md`。
